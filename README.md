...
| 이름 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;| 업무 |
|----------------------|------|
| 김윤진 | 백엔드: 주식 데이터 분석 및 계산 (이동 평균선, 골든크로스/데드크로스 신호) <br>이메일 알림 시스템: 주식 목표가 도달, 급등/급락, 거래량 급증 등의 알림 시스템 구현 / 이메일 발송 로직 |
| 팀장 <br>이성현 | 백엔드: 주식 데이터 DB 저장 및 관리 (MySQL) / 클라우드를 활용한 벡엔드 서버 배포 / 주식 정보 API 설계 및 구축 / DB 연결 및 데이터 처리 / 로그인/회원가입 시스템 구현 / 스케쥴러를 활용한 모니터링 모듈 도입 |
