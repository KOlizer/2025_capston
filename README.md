...
| 기능 구분           | 상세 내용 |
|--------------------|-----------|
| 로그인 & 회원가입  | 이메일 기반 회원가입 / 로그인 <br> 회원 정보 조회, 수정, 삭제 |
| 작업 관리           | 작업 추가 / 수정 / 삭제 / 조회 기능 <br> 작업에 담당자 지정 및 시작일 설정 가능 |
| 백로그              | 상태가 "완료"인 작업들을 자동으로 백로그 목록에 정리하여 보여줌 |
| 대시보드           | 대시보드 생성 / 이름 수정 / 삭제 가능 <br> 대시보드별 작업 구분 가능 |
| 동료 관리           | 동료 초대, 추가 / 삭제 / 조회 <br> 초대한 동료에게 이메일 자동 발송 |
| 캘린더              | 작업 시작일이 자동으로 캘린더에 표시됨 <br> 작업 삭제 시 해당 일정도 캘린더에서 제거 |
| 즐겨찾기           | 원하는 종목을 즐겨찾기에 추가하여 사이드바에서 빠르게 접근 가능 |
| 검색               | 주식 종목 검색 시 실시간 자동완성 및 상세 정보 팝업 제공 |
| 알람 기능           | 설정된 조건(가격, 거래량 등) 만족 시 이메일 알림 발송 |
| 새로고침 설정       | 자동 새로고침 주기를 사용자마다 개별 설정 가능 (로그인 유지 시 적용) |
| 다크모드           | 개인 설정에 따라 다크모드 적용 가능 (UI 전체에 반영) |
